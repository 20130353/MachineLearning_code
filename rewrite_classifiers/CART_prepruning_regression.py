...
# description: CART regression decision tree, it can solve continuous features,missing value
...
# regression utilize the MSE error to evaluate the rent point
...
class CART_regression():
    ...
    def _regerror(self,x,value):
        return sum([(each-value) ** 2 for each in x])
        ...
        for dim in range(dims):
            ...
            if type(feature_unique[0]).__name__ != 'str': #continuous type
                ...
                for inx,each in enumerate(split_points):
                    ...
                    reg_error = self._regerror(x_1_feature,each) + self._regerror(x_2_feature,each)
                    if reg_error < feature_best['gain']:
